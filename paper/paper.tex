...
完成坐标系统一后，采用 HEALPix 等面积分区方案（\texttt{nside}=64）将全天划分为 49,152 个天区。在每个天区内，以 $1\mathrm{arcsec}$ 为搜索半径，对测试星表与 Gaia DR3 恒星源进行空间匹配，并施加对应约束。对每个天区内所有成功匹配的恒星，在 J2000.0 历元下分别计算赤经与赤纬方向的位置差分，以及对应的自行差分，并采用中位数作为该天区的系统修正量。
