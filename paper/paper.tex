...
Eggl 等人（2020）以 Gaia DR2 的五参数解为参考，构建了区域性星表修正模型，并系统刻画了多种历史星表在小天体天体测量中的主要系统偏差结构。我们的目标是在 Gaia 参考框架下扩展适用范围并提供可复用实现，侧重于在更稳定参考基准上对既有结论进行一致性检验与工程化补全。鉴于星表修正方法对参考星表误差统计特性的高度敏感性，本文延续 Eggl 等人（2020）的基本思路，选用 Gaia DR3 作为参考星。
...
%% 是否包含自行信息的分类
%% 星表在小天体观测归档中的实际使用情况
不同历史时期的光学观测依赖于多种恒星参考目录。这些目录在观测历元、覆盖范围、源密度和天文测量内容上有所不同，反映了地面和空间天文测量发展的连续阶段。下面对本文所涉及的星表作简要概述。
...
基于上一章所述的去偏流程，本文针对选取的 19 个恒星天体测量星表，估计其相对于参考星表的恒星位置与自行系统修正量。修正量以 J2000 为参考历元，在 HEALPix 划分的天区内对匹配恒星差值进行稳健统计得到。本章首先汇总各星表修正量的全局稳健统计特性，随后以代表性星表为例展示修正量的统计分布形态与全天空间结构，并进一步给出修正结果的验证分析。
...
我们对每个星表在所有具有足够匹配样本的有效天区内分别统计位置修正$(\Delta \alpha_{\mathrm{J2000}}, \Delta \delta_{\mathrm{J2000}})$及自行修正$(\Delta \mu_{\alpha}, \Delta \mu_{\delta})$，并采用 MED 与 MAD 对其全局水平与离散度进行表征。如表~\ref{tab:global_corrections_j2000} 所示。在位置修正分量上，部分星表的全局中值已达 $\sim10^2$\,mas，并伴随 $\gtrsim10^2$\,mas 的离散度；例如 USNO-A2.0 的 $\Delta\alpha^\ast_{J2000}$ 的 MED 为 68.6 mas、MAD 为 107.6 mas；$\Delta\delta_{J2000}$ 的 MED 为 156.8 mas、MAD 为 124.6 mas。与此同时，也存在位置修正分布更为集中的星表（MAD 为数至十 mas 量级）。
...
图~\ref{fig:hist_usnoa2} 顶部面板显示，USNO-A2.0 的位置修正在赤经与赤纬方向均呈现明显的偏移：$\Delta\alpha_{J2000}$ 的全局中值为 $68.6$\,mas，$\Delta\delta_{J2000}$ 的全局中值为 $156.8$\,mas，分布整体偏向正值。底部面板显示，自行修正在两个分量上均存在系统性偏移，其中 $\Delta\mu_{\delta}$ 的中值（$3.31$\,mas\,yr$^{-1}$）高于$\Delta\mu_{\alpha}$（$1.36$\,mas\,yr$^{-1}$），且分布同样呈现非高斯形态。鉴于上述分布的非对称性与离群特征，因此后续统一采用 MED 与 MAD 作为稳健统计量。
...
\subsubsection{底片视场拼接效应 (Field-of-View Tiling Effects)}
...
对于 UCAC2 等采用带状扫描或重叠观测策略的星表，其系统性偏差通常表现为沿扫描方向分布的条带状结构。在 UCAC2 中可清晰观测到与 Eggl et al.\ (2020) 一致的“星等—位置”耦合现象，即在低银纬、高源密度区域修正幅度显著增大。该类偏差通常归因于早期 CCD 探测器的电荷转移效率（CTE）效应以及拥挤星场中的测量退化。
...
\subsubsection{历元传播导致的全球性梯度 (Epoch Propagation Induced Gradients)}

对于 USNO-A2.0 等未提供自行信息的星表，其系统性偏差主要体现为全天尺度的平滑梯度结构。这一特征与 Eggl et al.\ (2020) 的分析结论高度一致，即微小的自行系统差在长达数十年的历元差下被线性放大。

如图~* 所示，USNO-A2.0 在赤纬方向呈现出显著的南北不对称结构，其本质是银河系整体旋转及太阳运动在缺乏自行修正的静态坐标框架中投影形成的系统性伪影。类似的行为亦可在缺乏完整自行解的 Gaia DR1 中观察到，进一步强调了高精度自行信息在构建刚性天体测量参考系中的关键作用。需要指出的是，对于采用 FK4/B1950 参考体系的早期星表（如 AGK3、Yale 及 FK4 Catalogue），其在 J2000 历元下表现出的角秒级天区边界跳变，并不意味着其原始观测历元下已存在同等幅度的位置误差。在本文的处理流程中，相关修正量需通过约 50 年的时间基线从 B1950 传播至 J2000。在这一过程中，即便是百毫角秒量级的局部自行系统差，亦会被线性放大，并叠加到原本已存在的区域性不连续结构之上，从而显著增强其在 J2000 框架下的表现幅度。
...
\end{figure*}
在本工作中，我们沿用 Eggl et al.\ (2020) 提出的\textbf{（Basis Function Smoothing, BFS）}方法如图~\ref{fig:bias_ucac4_zoom_compare_n256}，对 HEALPix 网格上的离散修正量场进行处理。该方法直接作用于像素化修正量本身，其输出仍定义在 HEALPix 网格上，并不构建连续函数形式的插值模型，亦不依赖额外的物理先验。从方法论角度看，BFS 等价于在球面上对修正量场施加低通滤波，以抑制像素到像素之间的高空间频率不连续成分，同时保留系统性偏差在全天范围内的主导空间结构。
...
\end{figure*}
平滑处理前后修正量场连续性的变化通过相邻 HEALPix 天区之间的修正量跳变指标进行定量评估。如图~\ref{fig:bias_smooth}，BFS 平滑在所有分析星表中均显著降低了天区边界处的修正量 $\delta_{\text{jump}} (i, j)$ 。平滑后最大跳变值相对于原始值的比例 $R_{\mathrm{jump}}=\mathrm{MaxJump}_{\mathrm{after}}/\mathrm{MaxJump}_{\mathrm{before}}$ 通常小于 $0.1$ ，即原本在数百毫角秒至角秒量级的边界不连续性被有效抑制至更平滑的空间变化尺度。
\begin{figure*}[ht!]
\plotone{"../Figue/figure_Y_jump_distribution.png"}
\caption{平滑前后修正量邻域 $\delta_{\text{jump}} (i, j)$ 的累积分布对比}
\label{fig:bias_jump}
\end{figure*}
除极值指标外，我们进一步考察了跳变分布整体形态的变化。结果显示，跳变分布的高分位值（如第 95 百分位）在平滑后整体下降至原始水平的约 $10\%$，表明 BFS 平滑并非仅抑制个别异常边界，而是系统性地削弱了修正量场中的高空间频率成分。
