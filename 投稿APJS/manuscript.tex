...
\enddata
\tablecomments{MED denotes the median and MAD the median absolute deviation over all valid tiles, \textbf{defined as $\mathrm{median}(|x_i-\mathrm{median}(x)|)$}. Position corrections $\Delta\alpha_{\rm J2000}$ and $\Delta\delta_{\rm J2000}$ are in mas; proper motion corrections $\Delta\mu_\alpha$ and $\Delta\mu_\delta$ are in mas~yr$^{-1}$.}
\end{deluxetable*}
...
\caption{Cumulative distributions of inter-tile discontinuities before and after smoothing. \textbf{The CDF denotes the cumulative distribution function, $P(D_{\mathrm{disc}} \leq x)$.}}
\label{fig:bias_jump}
\end{figure}
...
As a complementary external test, this section compares the \textbf{Observed minus Calculated (O$-$C)} residuals before and after applying the catalog debiasing corrections, using the JPL Horizons DE441 planetary ephemeris \citep{Park2021DE441} as an independent dynamical reference.

The validation sample consists of 11\,731 absolute astrometric observations of the four Galilean satellites from the \textbf{NSDB archive}, spanning 1962--2019 and covering 8 reference catalogs across 19 observation files. Because some files contain segments reduced with multiple catalogs, observations are grouped by (file, catalog) pairs, giving 21 groups.
...
This work has made use of the \textbf{Natural Satellite Database (NSDB)}, maintained by the Laboratoire Temps Espace (LTE), which provides a comprehensive compilation of astrometric observations of planetary satellites. We are grateful to \textbf{those who collect and maintain these datasets} for their efforts.
